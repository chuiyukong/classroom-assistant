...
(function(){
  ...
  function filteredClasses(){return classes.filter(function(c){return (!el('log-grade').value||(c.grade||'unset')===el('log-grade').value)&&(!el('log-year').value||String(c.year)===el('log-year').value)&&(!el('log-semester').value||c.semester===el('log-semester').value);});}
  function classOptions(){var selected=el('log-class').value;el('log-class').textContent='';option('log-class','','全部班级');filteredClasses().forEach(function(c){option('log-class',c.id,c.name+' · '+(c.year?c.year+'年 '+c.semester:'未设置学期')+(c.deleted_at?' · 回收站':''));});el('log-class').value=filteredClasses().some(function(c){return c.id===selected;})?selected:'';}
  function loadClasses(){get('classes',function(err,live){if(err){put('log-message',err);return;}get('classes?deleted=1',function(error,deleted){if(error){put('log-message',error);return;}classes=live.classes.concat(deleted.classes);['grade','year'].forEach(function(key){var id='log-'+key,previous=el(id).value,values=[];el(id).textContent='';option(id,'',key==='grade'?'全部年级':'全部年份');classes.forEach(function(c){var v=key==='grade'?(c.grade||'unset'):String(c.year);if(values.indexOf(v)===-1){values.push(v);option(id,v,v==='unset'||v==='0'?'未设置':v);}});el(id).value=values.indexOf(previous)===-1?'':previous;});classOptions();});});}
  ...
  ['grade','year','semester'].forEach(function(key){el('log-'+key).onchange=classOptions;});
