...
**当前版本：1.2.0 · 阶段：机房测试 · 教师端：Windows 10/11 x64 · 部署：本地局域网**
...
新版流程见 [1.2.0 使用指南](docs/1.2.0使用指南.md)。AI 对话采集见 [下一阶段方案](docs/AI课堂对话采集方案.md)。
...
| 考勤签到 | 独立课堂快照、临时换座、长期请假、迟到、实时汇总和 CSV 日志 |
...
![新版考勤](docs/images/attendance-v120.png)
...
1. 在 [Releases](https://github.com/chuiyukong/classroom-assistant/releases) 下载 `ClassroomAssistant-v1.2.0-Win10-x64.zip` 并解压。
...
- 1.2.0 已通过 **48 项后端测试**，覆盖选座、签到并发与换座、点名来源、长期请假、权限、回收站和模板升级。
...
构建输出在 `dist/v1.2.0/`。课堂数据库、会话密钥、虚拟环境和本地测试产物不进入仓库；便携程序通过 Release 下载。
