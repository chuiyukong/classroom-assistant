...
项目从“替代打印座位表、逐个手写姓名”这一真实课堂需求出发，以班级、学生和座位安排为基础，逐步建设可维护的课堂辅助工具。当前版本专注于座位登记；随机点名、考勤、噪声提示和互动挑战已列入路线图，尚未实现。

**当前版本：1.1.0 · 阶段：机房测试 · 教师端：Windows 10/11 x64 · 部署：本地局域网**
...
![教师管理界面：班级、当前安排、设备状态与登记操作](docs/images/teacher.png)

### 学生登记

![学生登记界面：核对座位、查看已登记与停用设备](docs/images/student.png)

## 快速开始

1. 在 [Releases](https://github.com/chuiyukong/classroom-assistant/releases) 下载 `ClassroomAssistant-v1.1.0-Win10-x64.zip` 并解压。
...
| 版本 | 日期 | 主要进展 |
|---|---|---|
| 1.1.0 | 2026-09-06 | IP 防重复、同名处理、当前安排与存档、设备配置、学生备注、项目交接文档 |
...
| 随机点名 | 当前名单抽取、姓名/座位动画、小组筛选、不重复抽取、日志 |
| 考勤签到 | 每节课独立签到、未到/迟到状态、首次签到时间、教师补签、名单快照 |
...
- 1.1.0 已通过 **39 项后端测试**，覆盖并发、IP/同名限制、备注隔离、存档及数据迁移。
...
- **实际 Win7 Chrome/Firefox 版本、极域全班打开和机房防火墙仍需现场验收。**
...
构建输出在 `dist/v1.1.0/`。课堂数据库、会话密钥、虚拟环境和本地测试产物不进入仓库；便携程序通过 Release 下载。
