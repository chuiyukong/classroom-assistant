...
**当前版本：1.5.0 · 本地部署 · 教师机 Windows 10 x64 · 学生机 Win7 Chrome / Firefox**
...
1. 下载 `ClassroomAssistant-v1.5.0-Win10-x64.zip`，完整解压，关闭旧程序后双击 `ClassroomAssistant.exe`。
...
5. 日常考勤在考勤页“开始本节课堂”，设置签到时长，再发起签到。学生点座位、输入已登记姓名，临时换座选择原因。
6. **截止后仍可签到并自动迟到。** 未签到者看“签到超时：时长”，已签到者只看“已签到”。教师点击结束签到才停止接收，剩余未签到计缺勤。
7. 教师直接点座位纠正、补签或标长期请假；右侧工具栏处理长期换座申请、查询和导出日志。审批纠错完成后再结束本节课，之后日志只读。
...
| 登记开放状态 | 淡青色状态表示开放，灰色表示未开放；连接状态为独立提示，已连接不等于允许提交。 |
...
| 时限与迟到 | 服务端截止时间为准，截止时刻起记录迟到，迟到秒数从截止时间算起。已签到学生仅显示已签到，未签到者显示签到剩余或签到超时。倒计时到零不关闭；只有教师结束签到/结束课堂才停止。旧版本已关闭签到不会自动重开。 |
...
| 考勤纠正 | 教师点击座位弹窗补签/改状态，签到关闭后仍可操作，结束课堂后只读；补签按当前服务端时间算迟到，暂无任意补录到达时间。 |
| 日志查询 | 教师按班级、日期查询课堂快照与最终状态，导出 UTF-8 CSV（Excel/WPS 可打开）。暂无完整逐次纠正审计链。 |
| 随机点名 | 不需要开始课堂；没有当前考勤时直接用当前座位，已开展本节考勤时仅用到场名单（结束签到后仍用到场名单）。结束本节课后恢复当前座位。显示来源和候选人数，未到座位有状态提示。 |
...
浅青为已登记/已签到及正常开放，浅蓝为未签到，白色为空座，浅红为迟到，淡蓝紫为已换座，浅米色为长期请假，灰色为设备停用。颜色旁保留文字，避免只靠颜色判断。学生只能看到长期请假这一状态，不会看到教师备注。
...
| 临时换座审批失败 | 新座位有固定登记时不会覆盖他人；教师先核对名单并处理目标占用。临时签到成功不等于长期换座已获批。 |
| 程序/端口已在使用 | 先关闭旧实例；必要时在数据目录 settings.json 调整 port 并重启，重新发送学生网址。 |
| 名字很长 | 按最多六个汉字可完整显示设计；更长姓名仍保存，悬停座位可看全名。小屏横向滚动保留字号。 |
